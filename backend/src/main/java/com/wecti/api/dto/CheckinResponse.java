...
public record CheckinResponse(
    ...
    public static CheckinResponse de(Checkin checkin, Evento evento) {
        ...
        if (checkin.getSaida() != null) {
            long duracaoEvento = Duration.between(evento.getDataHoraInicio(), evento.getDataHoraFim()).toMinutes();
            long permanencia = Duration.between(checkin.getEntrada(), checkin.getSaida()).toMinutes();
